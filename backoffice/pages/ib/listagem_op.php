...
<?php
    ...
    //$xcrud->table('uc2_of'); 
		...
		$op=$_GET['id_op'];
	  	...
	  	$xcrud->join('id_of','ib_ofs','id_of'); // on profile.token_id = tokens.id
	  	

	  	$xcrud->label('ib_ofs.id_of','O.F.');
	  	$xcrud->label('ib_ofs.activo','O.F Activo');
...
$xcrud->columns('ib_ofs.id_of,activo,ib_ofs.activo'); 
    ...
    $xcrud->unset_add();
     ...
     $xcrud->set_var('nome_tabela','ib_ofs_prioridades');
...
$('#id_op').change(function() {
	window.location.href ='index.php?page=ib/listagem_op&id_op='+$(this).val();
...
function renumber_table(tableID) {
	...
	//alert(count);
	 $.getJSON('pages/ib/prioridades.php?prioridade='+count +'&id='+primary, function(data) {    
