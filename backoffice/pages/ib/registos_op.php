...
// selects
 ...
 if(isset($_GET['id_op'])){
  ...
  $op=$_GET['id_op'];
        ...
        $query4 = 'SELECT * FROM `ib_ofs_prioridades`, ib_of WHERE ib_ofs_prioridades.id_op='.$op.' and ib_ofs_prioridades.activo=1 and ib_ofs.activo=1 and
ib_ofs_prioridades.id_of=ib_ofs.id_of ORDER BY prioridade';
        $db4->query($query4);
        $result4 = $db4->result();
...
<?php
    ...
    if(isset($_GET['id_op'])){
    $xcrud->table('ib_ofs_prioridades'); 
    $xcrud->join('id_of','ib_ofs','id_of');
    }
    else
    {
    $xcrud->table('ib_ofs'); 
    ...
    $xcrud->label('ib_ofs.id_of','ID O.F.');
    $xcrud->label('ib_ofs.nome_of','Nome O.F.');
    $xcrud->label('ib_ofs.id_tiposdeproducao','Tipo de Produção');
    $xcrud->label('ib_ofs.ano','Ano');
    $xcrud->label('ib_ofs.semana','Semana');
    $xcrud->label('ib_ofs.sequencia','N.');
    $xcrud->label('ib_ofs.id_calibre','Calibre');
    $xcrud->label('ib_ofs.id_qualidade','Qualidade');
    $xcrud->label('ib_ofs.quantidade','Quantidade');
    $xcrud->label('ib_ofs.id_cliente','Cliente');
    $xcrud->label('ib_ofs.doc_cliente','Doc. Cliente');
    $xcrud->label('ib_ofs.id_of_materiaprima','O.F. Matéria Prima');
    $xcrud->label('ib_ofs.info_materiaprima','Inf. Matéria Prima');
    $xcrud->label('ib_ofs.id_of_materiaprima','O.F. Matéria Prima');
    $xcrud->label('ib_ofs.id_ordem_operacao','Ordem de Operações');
    $xcrud->label('ib_ofs.observacoes_gerais','Observações Gerais');
    $xcrud->label('ib_ofs.observacoes_privadas','Observações Privadas');
    $xcrud->label('ib_ofs.urgente','O.F. Urgente');
    $xcrud->label('ib_ofs.activo','O.F. Activa');
    $xcrud->label('ib_ofs.activo_materiaprima','O.F. Activa Matéria-Prima');
    $xcrud->label('ib_ofs.id_toleranciaproducao','Tolerâncias Produção');
    $xcrud->label('ib_ofs.producao_alteracoestolerancias','Alterações Tolerâncias'); 
    ...
    // relações - geral
    $xcrud->relation('ib_ofs.id_tiposdeproducao','uc2_tiposdeproducao','id_tiposdeproducao','nome_tiposdeproducao','id_tiposdeproducao!=1 and activo=1','id_tiposdeproducao');
    ...
    if(isset($_GET['id_op'])){
    $op=$_GET['id_op'];
    $xcrud->where('ib_ofs_prioridades.id_op='.$op);
    $xcrud->where('ib_ofs_prioridades.activo=1');
...
*/
    $xcrud->unset_add();
        ...
        $xcrud->columns('ib_ofs.nome_of,ib_ofs.urgente'); 
