<?php
    ...
    $xcrud->table('linguas');
    // nome da tabela
    $xcrud->table_name('Línguas');
    // mostra todos os ids excepto o id=1
    $xcrud->where('id_lingua !=',1);
    // nome das label
    $xcrud->label('cod_lingua','Código');
    $xcrud->label('nome_lingua','Nome');
    $xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('cod_lingua,nome_lingua,activo,principal');
	...
	$xcrud->set_var('nome_tabela','linguas');
    $xcrud->set_var('nome_campo','id_lingua');
