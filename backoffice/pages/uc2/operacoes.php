<?php
    ...
    // chama tabela
    $xcrud->table('uc2_operacoes');
    // nome da tabela
    $xcrud->table_name('Operações');
    // mostra todos os ids excepto o id=1
    //$xcrud->where('id_operacao !=',1);
    // nome das label
    $xcrud->label('cod_operacao','Código');
    $xcrud->label('nome_operacao','Nome');
    $xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('cod_operacao,nome_operacao,activo');
	...
	$xcrud->set_var('nome_tabela','uc2_operacoes');
    $xcrud->set_var('nome_campo','id_operacao');
