<?php
    ...
    // chama tabela
    $xcrud->table('uc2_lavacoes');
    // nome da tabela
    $xcrud->table_name('Lavações');
    // mostra todos os ids excepto o id=1
    $xcrud->where('id_lavacao !=',1);
    // nome das label
    $xcrud->label('cod_lavacao','Código');
    $xcrud->label('nome_lavacao','Nome');
    $xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('cod_lavacao,nome_lavacao,activo');
	...
	$xcrud->set_var('nome_tabela','lavacao');
    $xcrud->set_var('nome_campo','id_lavacao');
