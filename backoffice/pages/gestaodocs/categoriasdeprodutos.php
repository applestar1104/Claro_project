<?php
    ...
    // chama tabela
    $xcrud->table('docs_categoriasdeprodutos');
    // nome da tabela
    $xcrud->table_name('Categorias de Produtos');
    // mostra todos os ids excepto o id=1
    //$xcrud->where('id_tipodedocumento !=',1);
    // nome das label
    //$xcrud->label('cod_tipodedocumento','Código');
    $xcrud->label('nome_categoriadeproduto','Nome');
    //$xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('nome_categoriadeproduto,activo');
	...
	$xcrud->set_var('nome_tabela','docs_categoriasdeprodutos');
    $xcrud->set_var('nome_campo','id_categoriadeproduto');
