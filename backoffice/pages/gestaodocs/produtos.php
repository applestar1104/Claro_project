<?php
    ...
    // chama tabela
    $xcrud->table('docs_produtos');
    // nome da tabela
    $xcrud->table_name('Produtos');
    // mostra todos os ids excepto o id=1
    //$xcrud->where('id_produto !=',1);

   if(!protectThis("1") ){
    $xcrud->where('id_user in('.$user_id.')');
}

    
    // nome das label
    $xcrud->label('cod_produto ','Código');
    //$xcrud->label('nome_produto ','Nome');
    $xcrud->label('id_user','Clientes');
    $xcrud->label('id_categoriadeproduto','Categoria');
    $xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('nome_produto,id_categoriadeproduto,id_user,activo');
    $xcrud->relation('id_categoriadeproduto','docs_categoriasdeprodutos','id_categoriadeproduto','nome_categoriadeproduto','activo=1','id_categoriadeproduto asc',false);
	...
	$xcrud->set_var('nome_tabela','docs_produto');
    $xcrud->set_var('nome_campo','id_produto');
    ...
    $xcrud->button('../backoffice/pages/generatepdf.php?id_produto={id_produto}', false, 'glyphicon glyphicon-save-file','',array('target'=>'_blank'));
    ...
    $documentos = $xcrud->nested_table('id_produto','id_produto','pdfs','id_produto'); // 2nd level
