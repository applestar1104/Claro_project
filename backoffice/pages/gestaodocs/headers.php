<?php
    ...
    // chama tabela
    $xcrud->table('docs_headers');
    // nome da tabela
    $xcrud->table_name('Headers');
    // mostra todos os ids excepto o id=1
    //$xcrud->where('id_lingua !=',1);
    // nome das label
    $xcrud->label('nome_header','Nome');
    // colunas a mostrar na listagem
    $xcrud->columns('nome_header,conteudo_header,activo,principal');
	...
	$xcrud->set_var('nome_tabela','docs_headers');
    $xcrud->set_var('nome_campo','id_header');
