<?php
    ...
    // chama tabela
    $xcrud->table('tiposdefornecedores');
    // nome da tabela
    $xcrud->table_name('Tipos de Fornecedores');
    // mostra todos os ids excepto o id=1
    $xcrud->where('id_tipodefornecedor !=',1);
    // nome das label
    $xcrud->label('cod_tipodefornecedor','Código');
    $xcrud->label('nome_tipodefornecedor','Nome');
    $xcrud->label('observacoes','Observações');
    // colunas a mostrar na listagem
    $xcrud->columns('cod_tipodefornecedor,nome_tipodefornecedor,activo');
	...
	$xcrud->set_var('nome_tabela','tiposdefornecedores');
    $xcrud->set_var('nome_campo','id_tipodefornecedor');
