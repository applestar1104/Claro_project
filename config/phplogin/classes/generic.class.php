...
class Generic extends Connect {
	...
	public function displayMessage($error, $exit = true) {
		...
		if( !empty($error) ) :
			...
			if( $exit ) {
				include_once(cINC . 'index.php');
			$path = $_SERVER['DOCUMENT_ROOT'].'/'.FOLDER.'/ini.php';
			include($path);	
		?>
		<div class="page-footer" style="position:fixed; padding-left:0; width:100%; bottom:20px;"> <div class="page-footer-inner"> 2015 &copy; <?php echo $nomeapp; ?> - Desenvolvido por BonusCloud. Versão: <?php echo $vframework; ?>/<?php echo $vtemplate; ?>/<?php echo $vxcrud; ?>. </div> <div class="page-footer-tools"> <span class="go-top"> <i class="fa fa-angle-up"></i> </span> </div> </div>
		<?php		
			exit();
			}
