...
class Check extends Generic {
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['jigowatt']['restricted'])) :
			session_unset();
			session_destroy();
			header('Location: '. SITE_PATH . 'index.php?block=1');
			exit();
		endif;


		if (!empty($_SESSION['jigowatt']['level_disabled']) ) :
			session_unset();
			session_destroy();
			header('Location: '. SITE_PATH . 'index.php?block=2');
			exit();
		endif;
