...
class Forgot extends Generic {
	...
	private function validate() {
		...
		if(isset($_POST['reset']) && isset($this->key)) {
			...
			if($this->password != $this->password2) {
				...
				$this->error = '<div class="alert-danger alert alert-error">'._('Your passwords did not match, try again.').'</div>';
			} else if(strlen($this->password) < 5) {
				$this->error = '<div class="alert-danger alert alert-error">'._('Your password must be at least 5 characters.').'</div>';
			}
	...
	private function resetpw() {
		...
		if(empty($this->error) && isset($_POST['reset']) && isset($this->key)) {
			...
			echo "<div class='alert alert-success'>"._('Successfully reset your password')."<br/><button type='button' id='back-btn'  onClick=\"location.href='./index.php'\" class='btn'>
			<i class='m-icon-swapleft'></i> Back </button></div>";
	...
	private function reset_form() {
		...
		if(isset($this->key)) { 
			...
			<div class="input-icon">
				...
				<input class="form-control placeholder-no-fix" type="password" autocomplete="off" placeholder="Repetir Password"  id="password2" name="password2"/>
			</div>
		</div>
			...
			<button type="button" id="back-btn"  onClick="location.href='./index.php'" class="btn">
			<i class="m-icon-swapleft"></i> Back </button>
							<button type="submit" class="btn blue pull-right" name="reset"><?php _e('Reset Password'); ?></button>
						</div>
					</form>
				...
				$stmt   = parent::query("SELECT * FROM `login_users` WHERE `username` = :usernameEmail OR `email` = :usernameEmail;", $params);
				 ...
				 if( $stmt->rowCount() > 0 ) {
					...
					if(!parent::sendEmail($email, $subj, $msg, $shortcodes))
						...
						$this->error = "<div class='alert alert-success'>"._('Enviamos as instruções para o email.')."<br/><button type='button' id='back-btn'  onClick=\"location.href='./index.php'\" class='btn'>
			<i class='m-icon-swapleft'></i> Back </button></div>";

				} else { $this->error = "<div class='alert-danger'>
		<div class='alert alert-error'>"._("Nao encontramos o seu email na base de dados")."</div></div><button type='button' id='back-btn'  onClick=\"location.href='./index.php'\" class='btn'>
			<i class='m-icon-swapleft'></i> Back </button>"; }
			}
