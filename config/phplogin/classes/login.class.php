...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if (parent::getOption('disable-logins-enable')) {
			parent::displayMessage('<div class="alert alert-error" id="errophp">' . _('The admin has disabled logins.') . '</div>');
		}
		...
		if(isset($_POST['login'])) :
			...
			$this->browser = parent::secure($_POST['browser']);
	...
	private function isSecure() {
		...
		if(isset($_GET['e'])) :
			if (parent::getOption('block-msg-out-enable'))
				$this->msg = '<div class="alert alert-error" id="errophp">'.parent::getOption('block-msg-out').'</div>';
		endif;
	}
	...
	private function process() {
		...
		if(!parent::valid_token($this->token)) {
			$this->error = '<div class="alert alert-error" id="errophp">'._('Invalid login attempt').'</div>';
			return false;
		}
	...
	private function validate() {
		...
		if(empty($this->user)) {
			$this->error = '<div class="alert alert-error" id="errophp">'. (( $this->use_emails ) ? _('You must enter an email address.') : _('You must enter a username.')).'</div>';
			return false;
		}

		if(empty($this->pass)) {
			$this->error = '<div class="alert alert-error" id="errophp">'._('You forgot your password, silly.').'</div>';
			return false;
		}
		...
		$data_fim=$this->result['data_fim'];
		$data_inicio=$this->result['data_inicio'];

		$today = date("Y-m-d");

		if($data_fim){
		if($today>$data_fim){
		$this->error = "<div class='alert alert-error' id='errophp' >" . sprintf(_('Login expirado'), $username) . "</div>";
		return false;
		}
		}
	...
	if($data_inicio){
		...
		$this->error = "<div class='alert alert-error' id='errophp' >" . sprintf(_('Login por validar'), $username) . "</div>";
		return false;
		}
	}
		...
		if(!parent::validatePassword($this->pass, $this->result['password'])) {
			$username = $this->username_type;
			$this->error = "<div class='alert alert-error' id='errophp' >" . sprintf(_('Incorrect %s or password.'), $username) . "</div>";
		}
	
$_SESSION['jigowatt']['lingua'] = $this->result['lingua'];			
$tabela= $this->result['tabela'];
if($tabela!="")
{
 $campo= substr($tabela , 0, -1);

	$stmt2 = parent::query("SELECT id_".$campo." FROM ".$tabela." WHERE nif = ".$this->result['username']."");
$linha_array=$stmt2->fetch();
 $linha_tabela=$linha_array[0];
$_SESSION['jigowatt']['linha_tabela'] = $linha_tabela;
$_SESSION['jigowatt']['tabela'] = $tabela;
}
	...
	public function login() {
		...
		$_SESSION['jigowatt']['username'] = $this->result[$username];
		 ...
		 $_SESSION['jigowatt']['maquina'] =$this->browser;
...
$details = json_decode(file_get_contents("http://ipinfo.io/{$ip}/json"));
 $cidade=$details->city; // -> "Mountain View"
 $pais=$details->country; // -> "Mountain View"
 $local=$cidade.'('.$pais.')';
$maquina=$this->browser;
/*$maquina= "<script type='text/javascript'>
b = detect.parse(navigator.userAgent);
document.write('Browser: ' + b.browser.name + '-' +
        'Device: ' + b.device.type + '-' +
        'SO: ' + b.os.name );'</script>";
*/

$stmt = parent::query('INSERT INTO log_crud
(   
id_user,
accao,
data,
